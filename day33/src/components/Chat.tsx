  ...
  async function handleCommand(rawInput: string): Promise<boolean> {
    ...
    if (trimmed.startsWith('/review')) {
      ...
      try {
        ...
        if (args === 'last' || args === '') {
          try {
            diff = execSync('git diff HEAD~1', { cwd: process.cwd(), encoding: 'utf-8' });
          } catch {
            diff = '';
          }
        }
        if (!diff.trim()) {
          diff = execSync('git diff HEAD', { cwd: process.cwd(), encoding: 'utf-8' });
        }
        if (!diff.trim()) {
          setNotification('Нет изменений для ревью.\nИспользуй /review last — для последнего коммита\nИли сделай изменения в файлах.');
          return true;
        }

        const reviewer = new CodeReviewer(ragManager, currentModel);
        const result = await reviewer.review(diff, process.cwd());

        const lines: string[] = ['🔍 Code Review\n'];
        if (result.bugs.length > 0) {
          lines.push('🐛 Потенциальные баги:');
          result.bugs.forEach((b) => lines.push(`  • ${b}`));
          lines.push('');
        } else {
          lines.push('🐛 Потенциальные баги: не найдены\n');
        }
        if (result.architectural_issues.length > 0) {
          lines.push('🏗️ Архитектурные проблемы:');
          result.architectural_issues.forEach((a) => lines.push(`  • ${a}`));
          lines.push('');
        } else {
          lines.push('🏗️ Архитектурные проблемы: не найдены\n');
        }
        ...
        setNotification(`❌ Ошибка /review: ${err instanceof Error ? err.message : String(err)}`);
      } finally {
        setIsLoading(false);
      }
      return true;
...
🤖 Developer Assistant:
  ...
  /review                — ревью текущих изменений (RAG + LLM)
  /review last           — ревью последнего коммита
